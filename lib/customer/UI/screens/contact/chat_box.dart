...
class _ChatScreenState extends State<ChatScreen> {
  ...
  void initState() {
    ...
    context.read<GetChatBloc>().add(GetChatWithShip(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: BlocListener<GetIdBloc, OrderState> (
        listener: (context, state) {
          if(state is GotId) {
            receiverId = state.id;
          }
        }, child:
      BlocListener<GetChatBloc, OrderState>(
        listener: (context, state) {
          if (state is GetChatWithShipSuccess) {
      ...
      child: Row(
        children: [
          ...
          IconButton(
            ...
            onPressed: () {
              // Gửi tin nhắn
              widget.sendMessage(receiverId, _messageController.text);
              setState(() {
                messages.add({
                  'receiverId': widget.receiverId,
